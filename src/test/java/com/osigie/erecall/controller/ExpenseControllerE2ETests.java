...
public class ExpenseControllerE2ETests extends AbstractIntegrationTest {
  ...
  @Autowired private ExpenseDocumentRepository expenseDocumentRepository;
  ...
  class StatusTests {
    ...
    void givenValidId_whenGetStatus_thenReturn200() throws Exception {
      ...
      var documentId = objectMapper.readTree(response).get("data").get("documentId").asString();

      mockMvc
          .perform(
              get(EXPENSES_URL + "/" + documentId + "/status")
                  .header("Authorization", "Bearer " + token))
          .andExpect(status().isOk())
    ...
    void givenNoAuth_whenGetStatus_thenReturn403() throws Exception {
      mockMvc
          .perform(get(EXPENSES_URL + "/" + UUID.randomUUID() + "/status"))
          .andExpect(status().isForbidden());
    }
    ...
    void givenInvalidId_whenGetStatus_thenReturn404() throws Exception {
      ...
      mockMvc
          .perform(
              get(EXPENSES_URL + "/" + UUID.randomUUID() + "/status")
                  .header("Authorization", "Bearer " + token))
