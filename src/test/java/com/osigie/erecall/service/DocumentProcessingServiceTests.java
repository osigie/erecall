...
class DocumentProcessingServiceTests {
    ...
    private User createUser() {
        var user = User.builder().email("user@test.com").passwordHash("p").username("u")
    ...
    void handleDocumentSaved_withFileDocument_shouldGenerateUrlAndProcess() {
        var user = createUser();
        var docId = UUID.randomUUID();
        var document = ExpenseDocument.builder()
                .rawText("receipt")
        ...
        when(expenseDocumentRepository.findByIdWithCreator(docId)).thenReturn(Optional.of(document));
        when(fileStorageService.generatePresignedUrl("uploads/receipt.pdf", java.time.Duration.ofMinutes(5)))
    ...
    void handleDocumentSaved_withFileOnlyNoText_shouldUseDefaultPrompt() {
        ...
        when(expenseDocumentRepository.findByIdWithCreator(docId)).thenReturn(Optional.of(document));
        when(fileStorageService.generatePresignedUrl(anyString(), any())).thenReturn("https://presigned.url/doc");
        when(expenseService.queryWithMedia(anyString(), anyList(), eq(user), eq(docId)))
        ...
        assertThat(document.getProcessingStatus()).isEqualTo(DocumentProcessingStatus.PROCESSED);
        verify(expenseService).queryWithMedia(eq("Extract expense information from this document."), anyList(), eq(user), eq(docId));
    }
    ...
    void handleDocumentSaved_withException_shouldUpdateToFailed() {
        ...
        when(expenseDocumentRepository.findByIdWithCreator(docId)).thenReturn(Optional.of(document));
        when(expenseService.query(anyString(), any(), any())).thenThrow(new RuntimeException("LLM error"));
