...
public class ExpenseToolServiceImpl implements ExpenseToolService {
  ...
  private static String logCtx(ToolContext ctx) {
    return "documentId=%s, userId=%s"
        .formatted(ctx.getContext().get("documentId"), ctx.getContext().get("userId"));
  }

  @Override
  @Tool(
      description =
          "Save a new expense. Use when the user is recording, adding, or logging an expense.")
  public String saveExpense(
      @ToolParam(
              required = false,
              description = "Merchant or vendor name, e.g. Netflix, Shell, Shoprite")
          String merchant,
      @ToolParam(description = "Amount paid, e.g. 500, 29.99") BigDecimal amount,
      @ToolParam(description = "What the expense was for, e.g. 'Netflix monthly subscription'")
          String description,
      @ToolParam(description = "Category of the expense, e.g. ENTERTAINMENT, GROCERIES")
          ExpenseCategory category,
      @ToolParam(
              required = false,
              description =
                  "Date of the expense extracted from user input. Defaults to now if not provided.")
    ...
    log.info(
        "saveExpense called: {}, amount={}, description={}, category={}, merchant={}",
        logCtx(toolContext),
        amount,
        description,
        category,
  ...
  public List<String> getExpensesByDateRange(
      @ToolParam(
              description =
                  "Start of range (inclusive) — use start of day, e.g. 2024-01-15T00:00:00 for January 15")
          LocalDateTime startDate,
      @ToolParam(
              description =
                  "End of range (inclusive) — use end of day, e.g. 2024-01-15T23:59:59 for January 15")
          LocalDateTime endDate,
      ToolContext toolContext) {
    log.info(
        "getExpensesByDateRange called: {}, start={}, end={}",
        logCtx(toolContext),
        startDate,
        endDate);
    ...
    return expenseRepository
        .findByExpenseDocumentCreatorIdAndExpenseDateBetween(userId, startDate, endDate)
        .stream()
        .map(
            e ->
                "%s | %s | %s | %s | %s"
  ...
  public List<String> searchDatabase(
      @ToolParam(
              description = "Expense category filter (e.g. GROCERIES, DINING_OUT, ENTERTAINMENT)")
          ExpenseCategory category,
      ...
      ToolContext toolContext) {
    log.info(
        "searchDatabase called: {}, category={}, merchant={}, amount={}",
        logCtx(toolContext),
        category,
        merchant,
        amount);
    ...
    Specification<Expense> spec =
        Specification.where(ExpenseSpecification.belongsToUser(userId))
    ...
    return expenseRepository.findAll(spec).stream()
        .map(
            e ->
                "%s | %s | %s | %s"
                    .formatted(e.getExpenseDate(), e.getMerchant(), e.getCategory(), e.getAmount()))
        .toList();
  }
  ...
  public List<String> searchVector(
      @ToolParam(
              description =
                  "Natural language description to search for, e.g. 'things I bought at the grocery store'")
          String query,
      ToolContext toolContext) {
    log.info("searchVector called: {}, query={}", logCtx(toolContext), query);
