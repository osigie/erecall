...
public class DocumentProcessingService {
  ...
  public void handleDocumentSaved(DocumentSavedEvent event) {
    ExpenseDocument document =
        expenseDocumentRepository.findByIdWithCreator(event.documentId()).orElse(null);
    ...
    try {
      ...
      if (hasFile(document)) {
        String presignedUrl =
            fileStorageService.generatePresignedUrl(document.getFileUrl(), Duration.ofMinutes(5));
        ...
        response = expenseService.queryWithMedia(text, List.of(media), user, document.getId());
      } else {
        response = expenseService.query(document.getRawText(), user, document.getId());
      }
  ...
  private void updateStatus(
      ExpenseDocument document, DocumentProcessingStatus status, String response) {
